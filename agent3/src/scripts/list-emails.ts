...
async function listEmails() {
  ...
  for (const [threadId, threadEmails] of threads) {
    ...
    for (const record of threadEmails) {
      ...
      console.log(`To: ${JSON.stringify(email.to_emails)}`);
      if (email.cc_emails) console.log(`CC: ${JSON.stringify(email.cc_emails)}`);
      console.log(`Subject: ${email.subject}`);
      console.log('\nBody:');
      console.log('-'.repeat(40));
      console.log(email.body_text || email.body_html || 'No body');
      console.log('-'.repeat(40));
    }
