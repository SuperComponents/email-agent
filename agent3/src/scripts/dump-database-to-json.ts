...
async function dumpDatabase() {
  ...
  try {
    ...
    // Tables not available in new schema
    // console.log('🏷️  Fetching email tags...');
    // const allTags = await db.select().from(emailTags);
    
    // console.log('📚 Fetching knowledge base articles...');
    // const allKBArticles = await db.select().from(knowledgeBaseArticles);
    ...
    // Write to file
    const filename = `database-dump-${new Date().toISOString().split('T')[0]}.json`;
