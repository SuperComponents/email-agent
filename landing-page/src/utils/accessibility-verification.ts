...
export function verifyReducedMotionSupport(): boolean {
  ...
  try {
    ...
    console.log('Reduced motion preference:', mediaQuery.matches ? 'enabled' : 'disabled');
    ...
    const isReducedMotionWorking = mediaQuery.matches && 
      (animationDuration === '0.01ms' || animationDuration === '0s');
    
    console.log('CSS reduced motion implementation:', isReducedMotionWorking ? 'working' : 'needs verification');
...
export function simulateReducedMotion(enable: boolean): void {
  ...
  const style = document.getElementById('test-reduced-motion') || document.createElement('style');
