...
function measureAnimationPerformance() {
  ...
  function checkWillChangeUsage() {
    ...
    elementsWithWillChange.forEach((el) => {
      ...
      if (computedStyle.willChange !== 'auto') {
        willChangeCount++;
        console.log(
          `Element with will-change: ${el.tagName}`,
          el,
          `Value: ${computedStyle.willChange}`
        );
      }
    });
  ...
  function checkGPULayers() {
    ...
    console.log(
      `🎮 Elements likely using GPU acceleration: ${elements.length}`
    );
  ...
  function testScrollPerformance() {
    ...
    setTimeout(() => {
      ...
      console.log(
        `📊 Frame Drop Rate: ${((frameDrops / scrollEvents) * 100).toFixed(2)}%`
      );
    }, 10000);
