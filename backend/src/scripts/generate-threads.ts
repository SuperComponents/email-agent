...
import { generateCoherentThread } from './email-thread-generator.js';
...
async function insertEmailThread(
  ...
  const participantEmails = Array.from(new Set(emails.map(e => e.senderEmail)));
  ...
  for (const email of emails) {
    const toEmails = email.isCustomer ? ['support@gauntletairon.com'] : [emails[0].senderEmail];
  ...
  await db.update(threads)
    .set({ last_activity_at: currentTime })
