...
import { processEmail, type ProcessEmailResult } from 'agent3'
  ...
  try {
    ...
    const agentConfig = {
      ...
      escalationKeywords: [
        ...
        'unacceptable', 'terrible', 'horrible', 'awful',
        'cancel', 'refund immediately', 'switch', 'competitor',
        'frustrated', 'angry', 'disappointed', 'furious'
      ]
    }
    ...
    const agentResponse: ProcessEmailResult = await processEmail(threadId, logger)
...
app.post('/:id/regenerate', async (c) => {
  try {
    ...
    const enhancedResponse: ProcessEmailResult = await generateEnhancedDraftResponse(threadId)
...
//       }
    ...
    return errorResponse(c, error instanceof Error ? error.message : 'Failed to regenerate enhanced draft', 500)
  }
})

// Helper function to format action descriptions
function formatActionDescription(action: string, metadata: Record<string, any>): string {
  ...
  if (metadata.confidence_score || metadata.confidence) {
    ...
    details.push(`Confidence: ${(parseFloat(confidence) * 100).toFixed(1)}%`)
  }
  ...
  if (metadata.rag_sources_count > 0) {
